<div class="ms-2">
    ...
    @endif
    <div>
        @if ($employeeWorker->employeeWorkerContract === null)
            <form action="/workers/employees/{{ $employeeWorker->id }}/contract" enctype="multipart/form-data"
                method="POST">
                @csrf
                <div class="ms-2">
                    <div class="d-flex flex-row">
                        <div class="col-8">
                            <p><span class="text-muted">Дата приема на работу:
                                </span><b>{{ date('d-m-Y', strtotime($employeeWorker->employee_hiring_date)) }}</b>
                            </p>
            ...
            <div>
                <p class="text-muted">Дата приема на работу:
                    <span class="text-dark">
                        <b>{{ date('d-m-Y', strtotime($employeeWorker->employee_hiring_date)) }}</b>
                    </span>
                </p>
                <p class="text-muted">Дата заключения трудового договора:
                    <span class="text-dark">
                        <b>{{ date('d-m-Y', strtotime($employeeWorker->employeeWorkerContract->employment_contract_term_start)) }}</b>
                    </span>
                </p>
                <p class="text-muted">Окончание действия трудового договора:
                    <span class="text-dark">
                        <b>{{ date('d-m-Y', strtotime($employeeWorker->employeeWorkerContract->employment_contract_term_end)) }}</b>
                    </span>
                </p>
                <p class="text-muted">Срок трудового договора:
                    <span class="text-dark">
                        <b>{{ Carbon\Carbon::parse($employeeWorker->employeeWorkerContract->employment_contract_term_start)->diffInMonths(Carbon\Carbon::parse($employeeWorker->employeeWorkerContract->employment_contract_term_end)) }}
                            месяцев</b>
                    </span>
                </p>
                <h5 class="mt-3 mb-3">Трудовой договор</h5>
                <a href="{{ URL::asset('storage') }}/{{ $employeeWorker->employeeWorkerContract->employment_contract }}"
                    download="Трудовой-договор-сотрудника-{{ $employeeWorker->employee_surname }}-{{ $employeeWorker->employee_name }}-от-{{ $employeeWorker->employeeWorkerContract->employment_contract_term_start }}"
                ...
                <button id="edit-button" wire:click="$toggle('showInputs')"
                    ...
                    <a wire:click="edit({{ $employeeWorker->employeeWorkerContract->id }})">
                        Редактировать
                    </a>
                </button>
            </div>
            @if ($showInputs)
                <form action="/workers/employees/contract/{{ $employeeWorker->employeeWorkerContract->id }}"
                    ...
                    <div>
                        <button type="submit" wire:click="update({{ $employeeWorker->employeeWorkerContract->id }})" class="col-8 mt-4 ms-2 p-3 btn btn-outline-primary d-inline-block">
                            Сохранить изменения
                        </button>
                    </div>
                </form>
