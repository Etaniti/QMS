<div>
    ...
    <div>
        <div class="col-9">
            ...
            <table class="table table-hover mb-4">
                ...
                <tbody>
                    @foreach ($employeeWorkerSecondments as $employeeWorkerSecondment)
                        <tr>
                            <td class="align-middle">{{ $loop->iteration }}</td>
                            <td class="text-center align-middle">
                                {{ date('d-m-Y', strtotime($employeeWorkerSecondment->employee_secondment_term_start)) }}
                            </td>
                            <td class="text-center align-middle">
                                {{ date('d-m-Y', strtotime($employeeWorkerSecondment->employee_secondment_term_end)) }}
                            </td>
                            <td class="text-center align-middle">
                                {{ $secondmentTerm = abs(round((strtotime($employeeWorkerSecondment->employee_secondment_term_end) - strtotime($employeeWorkerSecondment->employee_secondment_term_start)) / 86400)) + 1 }}
                                дней
                            </td>
                            <td class="text-center align-middle">
                                <a href="{{ URL::asset('storage') }}/{{ $employeeWorkerSecondment->employee_secondment_order }}"
                                    download="Приказ-от-{{ date('d-m-Y', strtotime($employeeWorkerSecondment->employee_secondment_term_start)) }}">скачать</a>
                            </td>
                            <td class="text-center align-middle">
                                <a href="/workers/employees/secondments/{{ $employeeWorkerSecondment->id }}/delete"
            ...
            <div class="pagination mb-3">
                {!! $employeeWorkerSecondments->links() !!}
            </div>
        </div>
    ...
    @if ($showInputs)
    <div>
        <form action="/workers/employees/{{ $employee_worker_id }}/secondments" enctype="multipart/form-data"
            method="POST">
            @csrf
