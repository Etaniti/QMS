    ...
    <div>
        <div class="col-9">
            ...
            <table class="table table-hover mb-4">
                ...
                <tbody>
                    @foreach ($employeeSpecialist->employeeSpecialistLeaves as $employeeSpecialistLeave)
                        <tr>
                            <td class="align-middle">{{ $loop->iteration }}</td>
                            <td class="text-center align-middle">
                                {{ date('d-m-Y', strtotime($employeeSpecialistLeave->employee_leave_start)) }}
                            </td>
                            <td class="text-center align-middle">
                                {{ date('d-m-Y', strtotime($employeeSpecialistLeave->employee_leave_end)) }}
                            </td>
                            <td class="text-center align-middle">
                                {{ $secondmentTerm = abs(round((strtotime($employeeSpecialistLeave->employee_leave_end) - strtotime($employeeSpecialistLeave->employee_leave_start)) / 86400)) + 1 }}
                                @if ($secondmentTerm < 5)
                                    дня
                                @else
                                    дней
                            ...
                            <td class="text-center align-middle">
                                <a href="{{ URL::asset('storage') }}/{{ $employeeSpecialistLeave->employee_leave_request }}"
                                    download="Заявление-на-отпуск-от-{{ date('d-m-Y', strtotime($employeeSpecialistLeave->employee_leave_start)) }}">скачать</a>
                            </td>
                            <td class="text-center align-middle">
                                <a href="/specialists/employees/leaves/{{ $employeeSpecialistLeave->id }}/delete"
            ...
            <div class="pagination mb-3">
                {!! $employeeSpecialistLeaves->links() !!}
            </div>
        </div>
        ...
        @if ($showInputs)
            <form action="/specialists/employees/{{ $employeeSpecialist->id }}/leaves" enctype="multipart/form-data"
                method="POST">
                @csrf
