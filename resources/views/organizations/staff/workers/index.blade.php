...
<div class="container">
    <div class="d-flex justify-content-evenly flex-column">
        <div class="d-flex flex-row justify-content-between mt-4 mb-3">
            <h2>Рабочая специальность {{ $worker->worker_name }}</h2>
            <a href="/organizations/{{ $worker->organization_id }}/staff" class="row px-5 py-2 me-1 btn btn-outline-primary d-inline-block">К штатному расписанию</a>
        </div>
        ...
        <table class="table table-sm table-bordered table-hover">
            ...
            <tbody class="border-top-0">
                <tr>
                    <td class="text-center align-middle">{{ $worker->worker_name }}</td>
        ...
        @can('update-worker', $worker)
            <div class="row justify-content-center col-md-13 mt-5 mx-1">
                <a href="/staff/workers/{{ $worker->id }}/edit" class="mt-2 p-2 btn btn-primary d-inline-block">Редактировать</a>
                <a href="/staff/workers/{{ $worker->id }}/delete" class="mt-2 p-2 btn btn-outline-secondary d-inline-block">Удалить</a>
            </div>
        @endcan
        <div>
                ...
                <tbody>
                    @foreach ($worker->employeeWorkers as $employeeWorker)
                        <tr>
                            ...
                            <td class="text-center align-middle">{{ $employeeWorker->employee_surname }}</td>
                            <td class="text-center align-middle">{{ $employeeWorker->employee_name }}</td>
                            <td class="text-center align-middle">{{ $employeeWorker->employee_patronymic }}</td>
                            <td class="text-center align-middle"><a href="/workers/{{ $worker->id }}/employees/{{ $employeeWorker->id }}" class="text-primary"><button type="button"class="btn btn-outline-primary">Показать</button></a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        @can('create-employeeWorker', $worker)
            @if ($worker->worker_count > $worker->employeeWorkers->count())
                <div class="row justify-content-center col-md-13 mt-3 mb-5 mx-1">
                    <a href="/workers/{{ $worker->id }}/employees/create" class="p-2 btn btn-outline-primary d-inline-block">Добавить сотрудника</a>
                </div>
            @endif
        @endcan
